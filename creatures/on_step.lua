...
-- Spawn particles
local function spawnParticles(...) end
...
creatures.on_step = function(self, dtime)
	...
	if self.lifetimer > def.stats.lifetime and not (self.mode == "attack" and self.target) then
		...
		if not self.tamed or (self.tamed and def.stats.dies_when_tamed) then
			creatures.kill_mob(self.object, def)
		end
	end
	...
	if 
		...
		-- and is a ...
		and (
			-- search enemy hostile
			(self.hostile and def.combat.search_enemy) 
			-- follower
			or modes["follow"]
		) 
	...
	if current_mode ~= "attack" and current_mode ~= "follow" and
		...
		self.yawtimer > (modes[current_mode].update_yaw or 4) 
	then
		self.yawtimer = 0
		local mod = nil
